...
class KisAuth:
    def __init__(self):
        ...
        # ".\\marketbond\\kib_api\\"
        self.config_root = (
            "/code/marketbond/kib_api/"  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
            # "..\\"
        )
        ...
        self.token_tmp = (
            self.config_root + "KIS" + datetime.today().strftime("%Y%m%d")
        )  # 토큰 로컬저장시 파일명 년월일

        # 접근토큰 관리하는 파일 존재여부 체크, 없으면 생성
        if os.path.exists(self.token_tmp) == False:
